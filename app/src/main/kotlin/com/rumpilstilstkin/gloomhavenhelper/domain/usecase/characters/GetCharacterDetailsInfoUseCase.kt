...
class GetCharacterDetailsInfoUseCase @Inject constructor(
    ...
    operator fun invoke(characterId: Int) = characterRepository.getCharacterByIdFlow(characterId).map {
        CharacterFullInfo(
            generalInfo = it,
            nextLevelExperience = getNextLevel(it.level),
            isDonateAvailable = it.gold >= 10
        )
    }
}