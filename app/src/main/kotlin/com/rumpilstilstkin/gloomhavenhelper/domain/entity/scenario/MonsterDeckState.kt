...
data class MonsterDeckState private constructor(
    ...
    fun drawCard(deckName: String, picker: CardPicker = CardPicker.Random): DrawResult {
        ...
        val newDeck = if (card.needsShuffle) {
            this
        } else {
            val index = deckCards.indexOfFirst { it.cardId == card.cardId }
            if (index < 0) this
            else MonsterDeckState(
                byDeck + (deckName to (deckCards.subList(
                    0,
                    index
    ...
    fun toAvailableCards(): List<AvailableCard> =
        getRemainingCards().map { AvailableCard(deck = it.deckName, cardId = it.cardId) }
    ...
    companion object {
        fun create(cards: List<MonsterCard>): MonsterDeckState =
            MonsterDeckState(byDeck = cards.groupBy { it.deckName })
    }
}
